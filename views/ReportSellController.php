...
class ReportSellController extends Controller {
    ...
    public function actionMonthly() {
        ...
        switch (Yii::$app->request->get('mode')) {
            case 'xls':
                return $this->export($model, $dataProvider, $summary, 'ข้อมูลการสั่งซื้อ ' . $model->textMonthRange);
            default:
                $monthlyItems = $model->getQuery()
                        ->groupBy([
                            'rec_date',
    ...
    public function actionYearly() {
        ...
        switch (Yii::$app->request->get('mode')) {
            case 'xls':
                return $this->export($model, $dataProvider, $summary, 'ข้อมูลการสั่งซื้อ ตั้งแต่ ' . ($model->from_year) . ' ถึง ' . ($model->to_year));
            default:
                $monthlyItems = $model->getQuery()
                        ->addSelect([
    ...
    public function actionExcelTest() {
        ...
        $model->to_date = date('Y-m-t');
        $model->load(Yii::$app->request->get());
        $dataProvider = $model->summaryByProductTest();
        $coreQuery = $model->getQuery();
        $summary = $coreQuery->one();

        switch (Yii::$app->request->get('mode')) {
            case 'xls':
                $exportExcel = $model->getQuery()->addSelect([
                    'rec_date as date_sold',
                    'month(rec_date) as month_sold',
                    'info_publish',
                    'case when 478 in (folder.id, folder.folder_id) then "นิตยสาร" else "pocket book" end as folder_name',
                    'product.sku as sku',
                ...
                ->join('INNER JOIN', 'product_folder', 'product_folder.product_id = report_sell_product_daily.product_id')
                ->join('INNER JOIN', 'folder', 'folder.id = product_folder.folder_id')
                ->groupBy(['product_id', 'folder_name', 'rec_date'])
                ->orderBy(['rec_date' => SORT_ASC,])
                ->asArray()
                ->all();
                return $this->exportTest($model, $exportExcel, $summary, 'ข้อมูลการสั่งซื้อ ตั้งแต่วันที่ ' . ($model->from_date) . ' ถึง ' . ($model->to_date));
            default:
                $monthlyItems = $model->getQuery()
                        ->groupBy([
                            'rec_date',
    ...
    public function exportTest($model, $dataProvider, $summary, $title = null) {
        Yii::$app->response->format = Response::FORMAT_RAW;
        // $dataProvider->pagination = false;
        ...
        $sheet->setCellValueByColumnAndRow(1, $row, $title);
        $sheet->mergeCellsByColumnAndRow(1, $row, 8, $row);
        $sheet->getStyleByColumnAndRow(1, $row, 8, $row)->applyFromArray([
            'alignment' => [
                'horizontal' => Alignment::HORIZONTAL_CENTER,
            ],
            'font' => [
        ...
        $sheet->setCellValueByColumnAndRow(2, $row, 'เดือน');
        $sheet->setCellValueByColumnAndRow(3, $row, 'ปีที่พิมพ์');
        $sheet->setCellValueByColumnAndRow(4, $row, 'กลุ่มหนังสือ');
        $sheet->setCellValueByColumnAndRow(5, $row, 'SKU');
        $sheet->setCellValueByColumnAndRow(6, $row, 'ชื่อสินค้า');
        $sheet->setCellValueByColumnAndRow(7, $row, 'ยอดขาย');
        $sheet->setCellValueByColumnAndRow(8, $row, 'จำนวนสั่งซื้อ');
        for ($i = 1; $i <= 8; $i++) {
            $sheet->getStyleByColumnAndRow($i, $row)->applyFromArray([
                'alignment' => [
                    'horizontal' => Alignment::HORIZONTAL_CENTER,
        ...
        foreach ($dataProvider as $item) {
            ...
            $sheet->setCellValueExplicitByColumnAndRow(2, $row, ArrayHelper::getValue($item, 'month_sold'), DataType::TYPE_STRING);
            $sheet->setCellValueExplicitByColumnAndRow(3, $row, ArrayHelper::getValue($item, 'info_publish'), DataType::TYPE_STRING);
            $sheet->setCellValueExplicitByColumnAndRow(4, $row, ArrayHelper::getValue($item, 'folder_name'), DataType::TYPE_STRING);
            $sheet->setCellValueExplicitByColumnAndRow(5, $row, ArrayHelper::getValue($item, 'sku'), DataType::TYPE_STRING);
            $sheet->setCellValueExplicitByColumnAndRow(6, $row, ArrayHelper::getValue($item, 'name'), DataType::TYPE_STRING);
            $sheet->setCellValueExplicitByColumnAndRow(7, $row, Yii::$app->formatter->asDecimal(ArrayHelper::getValue($item, 'rec_amount'), 2), DataType::TYPE_STRING);
            $sheet->setCellValueExplicitByColumnAndRow(8, $row, Yii::$app->formatter->asDecimal(ArrayHelper::getValue($item, 'rec_count'), 0), DataType::TYPE_STRING);
            for ($i = 5; $i <= 6; $i++) {
                $sheet->getStyleByColumnAndRow($i, $row)->applyFromArray([
                    'borders' => [
                        'allBorders' => [
        ...
        $sheet->setCellValueExplicitByColumnAndRow(7, $row, Yii::$app->formatter->asDecimal(ArrayHelper::getValue($summary, 'rec_amount'), 2), DataType::TYPE_STRING);
        $sheet->setCellValueExplicitByColumnAndRow(8, $row, Yii::$app->formatter->asDecimal(ArrayHelper::getValue($summary, 'rec_count'), 0), DataType::TYPE_STRING);
        for ($i = 7; $i <= 8; $i++) {
            $sheet->getStyleByColumnAndRow($i, $row)->applyFromArray([
                'font' => [
                    'bold' => true,
